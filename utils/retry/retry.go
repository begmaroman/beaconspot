...
func DoFunc(attempts uint, sleep time.Duration, fn func(left uint) error) error {
	if err := fn(attempts - 1); err != nil {
		if attempts--; attempts > 0 {
			...
			time.Sleep(sleep)
			return DoFunc(attempts, 2*sleep, fn)
		}
		return err
	}
...
// Do tries to execute the function by its value, function can take variadic arguments and return multiple return.
